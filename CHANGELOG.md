...
### Fixed
- [MOB-1450] Address Book contacts and wallet metadata no longer leak between wallets with different seeds: the encryption keys for the iCloud-synced contacts and metadata files are now verified against the current wallet seed on every launch (a stale key left over from a previous wallet is replaced), and resetting the wallet now reliably clears every per-account metadata key regardless of the account's display name.
