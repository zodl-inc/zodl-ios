...
- [MOB-1802] Resuming an interrupted Keystone voting session no longer destroys local delegation data, so restarts pick up where signing left off instead of failing with a voting backend error; stale saved signatures are always re-verified against the stored signing data instead of trusted blindly. Voting errors — including a failed resume — now show an actionable message instead of raw database internals.
