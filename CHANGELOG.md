...
- [MOB-1802] Resuming an interrupted Keystone voting session no longer destroys local delegation data, so restarts pick up where signing left off instead of failing with a voting backend error; stale saved signatures are re-verified instead of trusted blindly, and residual incomplete-setup errors now show an actionable message instead of raw database internals.
