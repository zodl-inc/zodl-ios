...
### Fixed
- [Ironwood] Coinholder Polling can now restore a confirmed delegation for the small set of wallets that lost its local commitment randomness before 3.10.2. Recovery requires a complete match between the preserved database, the wallet's voting hotkey, the round parameters, and a freshly verified on-chain commitment tree; ambiguous or partial evidence is left untouched.
