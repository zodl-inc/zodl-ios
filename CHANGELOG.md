...
### Changed
- [MOB-1496] The Ironwood migration's background wakeups now make progress on every account that's actively migrating, not just your currently selected one — for example, a Keystone hardware account keeps advancing in the background even while a ZODL account is selected. Each wakeup still does at most one thing: either sync, or send a single transfer, never both.
- [MOB-1496] The migration Send Now button now sends exactly one overdue transfer per tap, the same limit background sends already follow, instead of sending every overdue transfer at once. Remaining overdue transfers stay scheduled, and tapping Send Now again sends the next one.
