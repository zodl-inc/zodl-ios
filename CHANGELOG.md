...
- [MOB-1458] Expired migration transfers are now rebuilt in place during recovery (the ZODL software wallet re-signs them automatically and lands straight on the migration schedule; Keystone hardware wallets re-sign only the rebuilt transfers by QR), with a full restart offered only as a fallback if they can't be rebuilt. Gated behind Ironwood network activation and not yet in a released build.
