...
- [MOB-1458] Confirming an Ironwood migration now asks for Face ID, Touch ID, or your device passcode first — on the Review Transfer screen for a single-transaction migration, on the Confirm Transfer Plan screen for a private multi-transaction migration (both ZODL software and Keystone hardware wallets), on each transfer you send yourself in manual mode, and when clearing a leftover Orchard remainder with "Migrate anyway". Declining leaves you on the same screen with nothing sent, and retrying after a failure asks again. Devices with no passcode set are unaffected. Gated behind Ironwood network activation and not yet in a released build.
