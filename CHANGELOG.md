...
### Fixed
- [MOB-1818] On the Send screen, the "Max" button no longer looks permanently unavailable while the wallet is still working out how much you can spend. Right after opening the app the spendable amount is briefly unknown, and the button used to sit greyed out next to a balance that was visibly still loading. It now shows the same loading spinner as the balance above it, so the two agree that the figure is on its way rather than that sending is blocked.
