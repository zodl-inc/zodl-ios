...
- [MOB-1497] The Ironwood migration now responds to broadcast failures instead of showing the same generic retry every time: if Tor can't connect on your very first send, you're offered a choice to retry or continue without it (with the same confirmation as turning it off manually); if Tor drops after transfers have already gone through, the migration pauses and only offers to retry, never a silent fallback; an unreachable broadcast server is retried on another server from the same provider automatically, with no prompt; and only once every server from that provider has failed are you offered a one-time, explained choice to broadcast through your own sync server instead. None of these choices are ever made silently in the background — background sends keep retrying on their own schedule (rotating to another server automatically when needed); only the Tor and sync-server choices wait for your next visit to the app. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1497] The Ironwood migration's Tor sheet now explains more before you commit: turning Tor off asks you to confirm first, with wording that reflects what your provider could observe on your migration path; the sheet (and, when it's skipped because Tor is already set up, the following screen) names the server that will actually receive your migration transfers, which is always different from the one you sync with; and a custom sync server now shows Tor as unavailable with the matching exposure explanation, instead of a toggle that couldn't work anyway. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1497] The Ironwood migration now responds to broadcast failures instead of showing the same generic retry every time: if Tor can't connect on your very first send, you're offered a choice to retry or continue without it (with the same confirmation as turning it off manually); if Tor drops after transfers have already gone through, the migration pauses and only offers to retry, never a silent fallback; an unreachable broadcast server is retried on another server from the same provider automatically, with no prompt; and only once every server from that provider has failed are you offered a one-time, explained choice to broadcast through your own sync server instead. None of this ever happens silently in the background — background sends simply wait for your next visit to the app. Gated behind Ironwood network activation and not yet in a released build.
