...
- [MOB-1497] The custom-server Tor sheet is redesigned: instead of the old unavailable notice, it now explains why your custom server can't use Tor and spells out the exposure risk in a "What are the risks?" card, then offers a clear choice — continue without Tor, or switch to a server that supports it (switching itself is wired up in a follow-up). Gated behind Ironwood network activation and not yet in a released build.
