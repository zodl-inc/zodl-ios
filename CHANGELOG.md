...
- [MOB-1671] The migration plan's transfer list now matches its intended spacing. Rows sat about 8pt too far apart, each step's icon rode slightly high against its own title, and the gaps around the titles, amounts and connecting lines were each a couple of points out — together making the list look loose and misaligned. Every step now sits on the designed rhythm, and a row with no amount to show keeps the same spacing as one that has it.
