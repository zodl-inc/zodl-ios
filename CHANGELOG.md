...
- [MOB-1580] A transaction that sends funds to your own address now shows the amount you sent instead of the network fee, in both the transaction list and the transaction detail — the same fix the Ironwood migration transfer already received, now covering any send to yourself. The amount counts only what you deliberately sent, not the change that returned to you, and is presented exactly like any other send: the fee is included in the total and also listed separately in the detail screen, so a send to yourself and a send to someone else show the same number for the same payment.
