...
### Fixed
- Coinholder Polling now recovers an ambiguously submitted delegation by checking its exact transaction hash or its persisted on-chain commitment, correctly reads delegation positions from older vote-server responses, and keeps the original selection and share mode on vote retries while verifying any recovered transaction. Reopening a poll also no longer discards persisted delegation setup needed to retry safely.
