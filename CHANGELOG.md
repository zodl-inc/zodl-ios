...
- [MOB-1511] Q3'26 design refresh for the migration flow: the Migrate Immediately note now matches the latest design, multi-round migrations show a "Round N" label on the transfer plan and the Home banner (with "of M" ready to light up once the engine estimate ships), a failed Tor connection posts a "Migration Failure" notification whose tap opens the warning sheet, the Progress screen's completed Split Balance row reads "Done", and the Migration Complete screen's warning button and success badge use the correct design tokens in dark mode. Gated behind Ironwood network activation and not yet in a released build.
