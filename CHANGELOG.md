...
- [MOB-1513] Fixed a rare case where migrating two accounts at the same time (for example a ZODL software wallet and a Keystone hardware wallet) could leave one migration's first transfer stalled instead of finishing on its own. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1513] The Home banner no longer repeats a full migration-state check roughly every two seconds on every synced wallet before Ironwood activates; that work now runs only once migration is actually available, cutting needless background processing.
