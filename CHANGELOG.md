...
### Changed
- [MOB-1513] After scanning the Keystone device's signature during an Ironwood migration, the scan screen now shows clear signing feedback instead of a silent camera view: the scan progress bar pins at 100% and a disabled "Signing…" button with a spinner replaces Cancel while the app finalizes and broadcasts (immediate migration) or applies and stores (scheduled and recovery migrations) the signed transactions. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1513] Spanish localization pass over the string catalog: the Ironwood migration flow's newest screens (How This Works, balance locking, Keystone firmware prompts, transfer review, notification titles, Tor protection card, resync banner) are now translated instead of falling back to English, drifted Spanish copy was realigned with the current English wording (transfer-plan and dust-remainder descriptions, progress banner, Keystone connected screen), and a Spanish-only formatting bug in the scheduled-plan description is fixed. Migration strings remain gated behind Ironwood network activation.
- [MOB-1513] Q3'26 Figma sync for the migration flow: the transfer plan drops the "Amounts randomized" footer and highlights only the Split Balance row as the current step, the manual transfer review uses the shorter design copy with "Once confirmed, this cannot be undone." under This Transfer (privacy-disclaimer callout removed), the resume screen drops the redundant "Transfer window missed" callout, the Skip and "Migrate anyway" warning buttons use the design's adaptive white fill, the round-mode Home banner reads "Round N of M · X of Y transfers done" without a percent segment, the notifications screen's Transfer Plan Changes wording matches the design, and the Migration Scheduled subtitle uses the primary text color. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1513] The expired-transfers recovery flow now shows the Confirm Transfer Plan screen before landing on Migration Scheduled — matching the designed flow — for both ZODL software and Keystone hardware wallets. Gated behind Ironwood network activation and not yet in a released build.
...
- [MOB-1511] Q3'26 design refresh for the migration flow: the Migrate Immediately note now matches the latest design, multi-round migrations show a "Round N" label on the transfer plan and the Home banner, with "of M" appearing once the wallet knows how many rounds are needed, a failed Tor connection posts a "Migration Failure" notification whose tap opens the warning sheet, the Progress screen's completed Split Balance row reads "Done", and the Migration Complete screen's warning button and success badge use the correct design tokens in dark mode. Gated behind Ironwood network activation and not yet in a released build.
...
- [MOB-1458] Expired migration transfers are now rebuilt in place during recovery — keeping the same funding note on a fresh schedule instead of requiring a full restart: the ZODL software wallet re-signs them automatically and lands straight on the migration schedule, while Keystone hardware wallets re-sign only the rebuilt transfers by QR, with a full restart offered only as a fallback if they can't be rebuilt. Gated behind Ironwood network activation and not yet in a released build.
...
- [MOB-1497] When a scheduled Ironwood migration transfer couldn't reach the Tor network in the background, ZODL now asks on your next app open whether to try again over Tor or continue without it — with the exposure risks spelled out on the sheet itself. Choosing to continue without Tor applies to the rest of the migration run. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1510] Signing with a Keystone device now requires firmware 3.0.0 or newer — older or version-less firmware is blocked with an update prompt before anything is broadcast.
- [MOB-1513] Debug-builds-only Advanced Settings actions to fast-reschedule and manually deliver migration transfers for QA testing.
- [MOB-1458] The Ironwood migration now asks for Face ID, Touch ID, or your device passcode before you commit funds: confirming a single-transaction migration on Review Transfer, confirming a private multi-transaction transfer plan (both ZODL software and Keystone hardware wallets), sending each transfer yourself in manual mode, rebuilding expired transfers from the recovery screen, and clearing a leftover Orchard remainder with "Migrate anyway". Declining leaves you where you were with nothing sent, and retrying a failed confirmation asks again. Devices with no passcode set are unaffected. Gated behind Ironwood network activation and not yet in a released build.

### Fixed
- [MOB-1513] Sending an immediate Ironwood migration with a Keystone hardware wallet no longer pops a bogus "Transaction Failed" sheet over the "migration sent" success screen. The broadcast had already succeeded — the success screen was mistakenly re-running the scheduled-transfer executor on appear, and its inevitable no-op result surfaced as an error. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1513] Large scheduled Ironwood migrations signed with a Keystone hardware wallet are capped at 32 transactions per QR signing round again: an oversized single batch could overload the device (a hardware crash was observed on Android with big batches), so the app now splits big migrations into several sign-and-scan rounds — the Sign Transaction screen shows "Round X of Y" and nothing is stored until every round is signed, so backing out midway safely restarts the whole signing later. Small migrations still sign in a single round exactly as before. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1513] Scheduled Ironwood migrations signed with a Keystone hardware wallet no longer fail after scanning the device's signature when the plan includes balance-preparation (note-split) transactions: the app now hands the signing bridge plain transaction ids it can digest and restores its own bookkeeping labels afterwards, so the whole signed batch stores and the migration schedule proceeds. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1513] Confirming an immediate ("Migrate Immediately") Ironwood migration with a Keystone hardware wallet no longer loops forever between the Review and Sign Transaction screens after scanning the device's signature: the single sweep transaction now signs over the same proven Keystone flow as a regular send, the transfer is actually broadcast, and the success screen appears with the transaction id. The "Migrate anyway" dust sweep on the Migration Complete screen rode the same broken path for Keystone wallets and gets the same fix. If the broadcast genuinely fails, the Review screen now shows the failure sheet with a Retry instead of silently returning, and a device on too-old firmware sees the firmware-update prompt with the correct required version. Gated behind Ironwood network activation and not yet in a released build.
- [#1936] The smart banner's auto-shielding help sheet no longer shows the internal placeholder text "autoShieldingHelpContent" — it now presents proper localized content (English and Spanish) in the same layout as the other banner help sheets.
- [#1937] The Settings → Recover Funds result messages are now localized instead of hardcoded English, and the Tor-required toast shows the full "enable Tor Protection in Advanced Settings" explanation instead of a terse "Tor required".
...
- [MOB-1513] Tapping the "Migration Required" banner right after restoring a wallet no longer shows an error while the wallet finishes preparing — the migration screen quietly keeps loading and retries for up to a minute, so it opens straight into the transfer plan (or the immediate sweep) as soon as your funds are ready. If preparation genuinely can't finish in that window, the usual "couldn't load" retry prompt appears, exactly as before. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1513] The migration Transfer Plan, Migration Progress, and Resume screens now show a real estimated time for each upcoming transfer ("Ready now", "in ~45 mins", "in ~6 hours") instead of every row falling back to "~10 mins". Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1513] Confirming a migration Transfer Plan now shows a spinner on the Confirm button while the commit runs, and tapping Confirm again while it's in flight no longer starts a second, conflicting commit that ended in an error sheet. The Keystone review step's Confirm gets the same loading and double-tap protection. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1513] Confirming a migration Transfer Plan no longer freezes the app for several seconds: everything is signed at once at confirm, the app goes straight to Migration Scheduled, and the balance split's first transaction is sent immediately in the background — with any network hiccup retried automatically instead of surfacing an error over the confirmation. Keystone commits follow the same flow after their signing ceremony. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1513] Fixed migration transfer-plan confirmation always reporting a refreshed plan and never committing the schedule.
- [MOB-1513] The immediate ("Migrate Immediately") Ironwood migration now stays completely quiet while its single sweep transaction waits to be mined: the Home banner shows nothing (no leftover "Migration Required" title and no progress ring), reopening the app doesn't pop a migration status screen, and background wakeups stop rescheduling for it. If that transaction later expires, "Migration Required" is offered again as usual. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1513] The "Migration Required" banner now appears within a couple of seconds of your migratable balance becoming available — for example right after restoring a wallet, even when no other banner was on screen — instead of sometimes staying hidden until another event happened to re-check it. The re-check also stays fully idle before Ironwood activates on the network, so it costs nothing on wallets that can't migrate yet. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1513] The transaction history now shows the real amount that crossed to Ironwood for a migration transfer — in both the history list and the transaction detail screen — instead of collapsing it to just the fee. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1513] Fixed a rare case where migrating two accounts at the same time (for example two Keystone hardware wallets) could leave one migration's first transfer stalled instead of finishing on its own. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1513] The "Migration Required" banner now shows up reliably right after a wallet restore finishes, instead of the currency-conversion prompt taking its place.
- [MOB-1513] The "Migration Required" banner now shows up reliably right after a wallet restore finishes, instead of the currency-conversion prompt taking its place. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1513] The transfer plan and progress screens now show the balance split as its own immediate step with the correct total amount, and number every transfer. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1513] Migration progress rows now show real time estimates from the schedule instead of six-hour placeholders. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1513] Migration progress rows now reflect the engine's live per-transaction state (signed, broadcast, mined, blocked), including right after a refresh reschedules transfers. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1513] Migration progress rows and totals no longer show placeholder zero amounts after a restore mid-migration — statuses, ETAs, and counts come from the engine's live transaction statuses, and unknown amounts are omitted instead of shown as 0 ZEC. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1513] Switching between the ZODL and Keystone accounts now always shows the selected account's transaction history — a slow in-flight refresh for the previous account can no longer overwrite it, and connecting a Keystone refreshes the list immediately.
- [MOB-1513] Running migrations on both the ZODL and Keystone wallets at the same time is now fully supported: each wallet keeps its own reminder notifications, its own background wake-ups, and opening the migration screen for one wallet no longer disturbs the other wallet's in-flight signing. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1513] The Move to Ironwood entry screen now shows the fiat-currency value of the balance being migrated alongside the ZEC amount, when an exchange rate is available. Gated behind Ironwood network activation and not yet in a released build.
...
### Changed
- [MOB-1496] Ironwood migration transfers now broadcast through a different server provider than the one you sync with, for the whole run — except when you've chosen a single custom server, or you're on testnet, where both intentionally share it. Automatic server selection stays within your migration's original sync-provider family for as long as the migration is active, and Server Setup warns you before a manual switch would undo that separation, since syncing and broadcasting through the same operator could let it link the two together. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1496] The Ironwood migration's background wakeups now make progress on every account that's actively migrating, not just your currently selected one — for example, a Keystone hardware account keeps advancing in the background even while a ZODL account is selected — with each wakeup still doing at most one thing: either sync, or send a single transfer, never both. The migration Send Now button follows the same one-transfer-per-tap limit instead of sending every overdue transfer at once; remaining overdue transfers stay scheduled, and tapping Send Now again sends the next one. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1496] The Ironwood migration's privacy pause between syncing and sending is enforced by the SDK on both sides and applied consistently on every send path: syncing pauses briefly after any migration send — scheduled, immediate, or Send Now — and sending pauses briefly after a sync finishes, using the SDK's own timing throughout (including an accurate "about N mins" wait note that no longer briefly shows "0" when reopening the screen). Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1496] The Ironwood migration progress and status screens now show your real transfer history, amounts, and totals once a schedule is confirmed, instead of placeholder counts. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1497] Tor protection for the Ironwood migration is on by default whenever it's available, the migration broadcast server is now chosen at random from among your sync provider's peers instead of by a speed benchmark (so nothing is probed over the network before you've confirmed anything), and a manually entered server address is now recognized as belonging to a known provider whenever it genuinely does, rather than always being treated as a separate custom server. The network choice is now locked in as soon as you resolve the Tor step, instead of at your first scheduled send. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1497] The Ironwood migration's Tor sheet now explains more before you commit: turning Tor off asks you to confirm first, with wording that reflects what your provider could observe on your migration path; the sheet (and, when it's skipped because Tor is already set up, the following screen) names the server that will actually receive your migration transfers whenever that's a different server than the one you sync with; and a custom sync server now shows Tor as unavailable with the matching exposure explanation, instead of a toggle that couldn't work anyway. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1497] The Ironwood migration now responds to broadcast failures instead of showing the same generic retry every time: if Tor can't connect on your very first send, you're offered a choice to retry or continue without it (with the same confirmation as turning it off manually); if Tor drops after transfers have already gone through, the migration pauses and only offers to retry, never a silent fallback; an unreachable broadcast server is automatically swapped for another one from the same provider, then only offers to retry; and only once every server from that provider has failed are you offered a one-time, explained choice to broadcast through your own sync server instead. None of these choices are ever made silently in the background — background sends keep retrying on their own schedule (rotating to another server automatically when needed); only the Tor and sync-server choices wait for your next visit to the app. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1497] Migration copy is synced with the latest designs: the Enable Tor Protection sheet now explains that Tor prevents linking your balance (immediate path) or the migration amounts (scheduled path) to your IP address, the custom-server wording no longer names the server host, and the migration notifications screen now mentions pre-signed transfer expiry. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1497] The custom-server Tor sheet is redesigned: instead of the old unavailable notice, it now explains why your custom server can't use Tor and spells out the exposure risk in a "What are the risks?" card, then offers a clear choice — continue without Tor, or switch to a server that supports it. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1497] The custom-server Tor sheet's Switch Server button now leaves the migration flow and opens Server Setup — nothing is saved for the abandoned attempt, so re-entering the migration re-evaluates the Tor choice from scratch. Migration screens also no longer print which broadcast server will receive your transfers: the Transfer Plan and Review Transfer footers are retired along with the Tor sheet's own disclosure line, per the new designs. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1497] The migration entry screen's immediate-mode warning and the manual transfer review's privacy note are now shown as a titled "Privacy Disclaimer" card, matching the latest designs — the manual review's description also now asks you to review and confirm to send the transaction. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1497] When resuming a stalled Ironwood migration transfer, ZODL now calls out that its transfer window was missed, alongside the existing delay note; the transfer plan screen marks its first, not-yet-confirmed step with a neutral check instead of a numbered one — a green check is now reserved for steps that have actually completed — titles that step "Split Balance", and once again explains that transfer amounts are randomized to reduce linkability; and a manually reviewed transfer's success screen now says your ZEC was "sent" rather than "migrated" (a fully completed migration run still reads "migrated"). Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1496] Keystone hardware-wallet migration signing now matches the final migration engine: preparing your notes to migrate can take several preparation transactions instead of just one, and signing, storing, and retrying after an interruption all handle any number of them correctly in the same QR session. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1496] A single Ironwood migration round may not cover your whole balance (very large wallets, or new funds arriving mid-migration) — acknowledging a finished round no longer permanently dismisses the migration banner when more remains: it reappears so you can start the next round. A background wakeup that finishes such a round now sends a "Migration batch finished" notification instead of "Migration complete" to reflect that more remains. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1490] Casting a coinholder vote now runs entirely on the voting engine's one-shot commit: the proof, tally-share encryption, signature, and crash-recovery state are produced together inside the engine, share submission times come from the engine's shared scheduling policy, and recovery after an interruption resumes from engine state. No visible UI changes; vote submission is more robust to crashes mid-flow.
- [MOB-1458] Coinholder voting now runs on the upstream `zcash_voting` 1.0 engine via the updated SDK: delegation reads eligible notes and keys from the wallet database directly (the wallet seed and viewing keys no longer pass through the voting client), and each account's voting hotkey is reconstructed deterministically from its stored voting phrase. No visible UI changes.
...
- [MOB-1496] Fixed Migrate Immediately silently signing a conflicting transfer when a note split was still needed on confirm — the immediate transfer never needed a split in the first place, and no longer attempts one. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1496] Fixed the Transfer Plan and Review Transfer screens silently committing nothing when the migration engine couldn't prepare a schedule — they now show the same failure screen used elsewhere in migration, with a working Retry, instead of appearing to succeed with an empty plan. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1496] Fixed a rare case where a note split's broadcast reached the network but the app's own record of it failed to save: confirming used to retry with a brand-new, conflicting split over the same funds. It's now recognized as already sent, and the migration plan finishes normally. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1496] Fixed Keystone hardware-wallet signing setup failing silently on the Transfer Plan and Review Transfer screens — a problem preparing the QR signing session now shows the failure screen with Retry instead of leaving you on a signing screen with nothing to scan. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1496] Fixed Keystone hardware-wallet signing showing a false "Migration Scheduled" success screen when storing the just-signed transfers actually failed — nothing was stored and no schedule was recorded. The attempt is now safely abandoned instead, returning you to Transfer Plan / Review Transfer to confirm and sign again. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1496] Fixed double-tapping Close on the migration Sending screen (after using Send Now) sometimes popping back two screens instead of one, landing outside the migration flow instead of on the status screen. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1496] Fixed migration completion acknowledgment: it's now tracked per account (instead of wallet-wide, which could suppress a second account's own completion banner) and can no longer wipe an active run's records if it's reached before the migration has actually finished. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1496] Fixed migration state reconciliation to cover every account with an active migration, not just your currently selected one — a Keystone hardware account's stale state used to go unrefreshed whenever a different account was selected. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1496] Fixed an abandoned migration setup (closing out before confirming a plan) leaking a server-privacy pin forever — Server Setup would keep warning about a manual switch, and automatic server selection would stay pinned, even though nothing was actually migrating. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1496] Fixed the scheduled migration notification occasionally showing the wrong transfer number (e.g. "Transfer 1" instead of the actual next transfer) when it's ready to send. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1496] Fixed the app failing to launch (with a "Failed to initialize the SDK" error) when opened during the brief privacy window right after a migration transfer — it now finishes launching normally and resumes syncing automatically once the window ends. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1496] Fixed syncing sometimes staying stopped for the rest of the session after a migration transfer failed to send — balances could look stale until the app was backgrounded and reopened. Syncing now resumes right away. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1496] Fixed background migration delivery being silently and permanently disabled if its background task happened to run during a cold app launch, or if the app was closed while that task was still working — it now correctly reschedules itself in both cases instead of going dark. Gated behind Ironwood network activation and not yet in a released build.
...
- [MOB-1496] Fixed "Send now"'s private countdown window leaving syncing paused for the rest of the session if you left that screen some other way than Cancel or letting it finish — for example tapping a migration notification while the countdown was running. Syncing now resumes correctly either way. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1496] Fixed automatic server selection being able to land on an active Ironwood migration's broadcast server through Settings' Save button — Automatic mode now stays within your migration's sync-provider family the same way background automatic selection already did, whether it's applying a fresh pick or a previously recommended one. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1496] Fixed the home migration banner losing track of progress after switching accounts — it now follows the newly selected account's migration state right away instead of silently staying on the previous account's status until you leave and return to Home. Gated behind Ironwood network activation and not yet in a released build.
...
- [MOB-1497] Fixed the Ironwood migration's Tor sheet and transfer-plan/review screens claiming your migration transfers go to "a different server than the one you sync with" even when they don't — on testnet, and in the rare case no other-provider server is available, migration already broadcasts through your sync server, so that line no longer appears for those cases. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1497] Fixed the Ironwood migration's note-split step not actually offering the broadcast-failure choices described above: a Tor or broadcast-server failure while a note split was in progress showed only a plain retry sheet instead of the first-send Tor choice, the automatic server rotation, or the eventual sync-server fallback. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1497] Fixed the Ironwood migration's waiting/stalled screens giving no sign that a stall was caused by Tor: the migration status screen's resume view and the home banner's "Transfer waiting" state now say so explicitly whenever the most recent send failed because Tor couldn't be reached, instead of looking identical to any other pause. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1497] Fixed the Ironwood migration's broadcast-failure sheet on the Sending and note-split screens: tapping "Proceed without Tor" on the first-send Tor choice did nothing, because the confirmation it's supposed to show was trying to appear over an already-presented sheet. That confirmation now actually appears. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1497] Fixed the Ironwood migration's transfer-plan screen not offering the broadcast-failure choices on its own silent note split: a Tor or broadcast-server failure there showed only a plain retry sheet instead of the first-send Tor choice, the automatic server rotation, or the eventual sync-server fallback — the same handling the Sending and note-split screens already had. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1497] Fixed the transfer-plan screen's silent note split not counting toward a scheduled migration run's "first send" status: a Tor failure partway through a run, right after that run's split had landed, could still show the first-send choice instead of holding and offering only a retry. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1497] Fixed the Ironwood migration's custom-server Tor sheet silently turning Tor off as your saved preference when you acknowledged it — it no longer changes that preference at all, so switching to a regular server later keeps whatever Tor choice you'd actually made. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1497] Fixed the Ironwood migration silently skipping the Tor-unavailable notice for a custom sync server if Tor setup had already been completed once — entering migration with a custom server now always shows that notice, so you're never routed around it without being told Tor isn't available. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1497] Fixed the Ironwood migration's broadcast-failure handling reacting to failures that happened before a transfer or note split was ever sent — a local problem such as preparing your signing key or saving a signed note split could still rotate your broadcast server or trigger its one-time consent prompt, even though nothing was actually broadcast. Those local failures now just show the plain retry sheet, with no server change and no prompt. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1497] Fixed a migration setup abandoned by killing the app: it no longer keeps pinning automatic server selection or warning about an active migration indefinitely. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1497] Fixed entering the Ironwood migration flow occasionally appearing to hang with no spinner: entering the migration flow no longer waits behind unrelated in-flight wallet operations, such as another account's transaction broadcast. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1497] Background Ironwood migration broadcasts now pause syncing first, the same way every other migration send already does, instead of relying on the send itself to reject the overlap; when a sync happens to be running at the same moment anyway (including the background wake that started it), it's now retried quietly instead of being counted as a broadcast-server failure. Gated behind Ironwood network activation and not yet in a released build.
- [MOB-1497] Fixed the Ironwood migration's Tor sheet showing a cramped gap before the "Got it" button whenever the broadcast-server disclosure line doesn't appear (testnet, or when broadcast and sync already share a server) — that gap now matches the spacing used when the disclosure line is shown. Gated behind Ironwood network activation and not yet in a released build.
