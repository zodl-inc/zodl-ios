...
- [MOB-1798] Coinholder Polling now recovers an ambiguously submitted delegation by checking its exact transaction hash or its persisted on-chain commitment, and vote retries keep the original selection and share mode while verifying any recovered transaction. Reopening a poll also no longer discards persisted delegation setup needed to retry safely.
