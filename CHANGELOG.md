...
### Changed
- [MOB-1496] The Ironwood migration's background wakeups now make progress on every account that's actively migrating, not just your currently selected one — for example, a Keystone hardware account keeps advancing in the background even while a ZODL account is selected. Each wakeup still does at most one thing: either sync, or send a single transfer, never both. Dormant until the migration SDK ships.
- [MOB-1496] The migration Send Now button now sends exactly one overdue transfer per tap, the same limit background sends already follow, instead of sending every overdue transfer at once. Remaining overdue transfers stay scheduled, and tapping Send Now again sends the next one. Dormant until the migration SDK ships.
- [MOB-1496] The Ironwood migration progress and status screens now show your real transfer history, amounts, and totals once a schedule is confirmed, instead of placeholder counts. Dormant until the migration SDK ships.
- [MOB-1496] The Ironwood migration's privacy pause between syncing and sending is now enforced by the real SDK on both sides: syncing pauses briefly after a migration send, and sending pauses briefly after a sync finishes, using the SDK's own timing throughout (including the "about N mins" wait note). Dormant until the migration SDK ships.
- [MOB-1496] Ironwood migration transfers now broadcast through a different server provider than the one you sync with, for the whole run (except when you've chosen a single custom server, or you're on testnet, where both intentionally share it) — and automatic server selection stays within your migration's original sync provider for as long as the migration is active, so it can't silently undo the separation.
