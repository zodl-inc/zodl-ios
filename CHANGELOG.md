...
### Fixed
- [Ironwood] Coinholder Polling can now restore confirmed delegation bundles for the small set of wallets that lost local commitment randomness before 3.10.2. Each recovered bundle must match the preserved database, the wallet's voting hotkey, the round parameters, and a freshly verified on-chain commitment tree; missing bundles continue through normal delegation, while ambiguous evidence is left untouched.
