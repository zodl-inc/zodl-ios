...
/// Produced by the SDK's one-shot vote commit. Recovery state, encrypted
/// shares, and share secrets stay inside the voting crate — the app only
/// carries the fields the cast-vote submission needs.
struct VoteCommitmentBundle: Equatable, Sendable {
    ...
    let rVpkBytes: Data
    /// SpendAuth signature over the canonical cast-vote sighash, produced by
    /// the one-shot commit.
...
/// A committed vote reconstructed from crate recovery state.
/// The share payloads carry the currently stored vote-commitment tree
/// position — record the confirmed position first for chain-ready payloads.
struct RecoveredCommittedVote: Equatable, Sendable {
    let bundle: VoteCommitmentBundle
    let sharePayloads: [SharePayload]
    let vcTreePosition: UInt64

    init(bundle: VoteCommitmentBundle, sharePayloads: [SharePayload], vcTreePosition: UInt64) {
        self.bundle = bundle
...
/// Carried out of the one-shot vote commit (`VoteCommitmentBundle.voteAuthSig`).
