...
extension VotingCoordFlow {
    ...
    private static func prepareFreshRound(
        ...
        // bundle rows exist, `roundResumeDecision` keeps them off it whenever a
        // probe or local material says the round is worth keeping, and
        // `loadExistingRoundSetup` makes a failed read propagate rather than
        // read as an empty round. This is the backstop for that invariant: the
        // cost of a redundant check is one database read, the cost of a missed
        // one is an unvotable round, and the asymmetry is the whole argument
        // for keeping it.
        if try await hasDelegationWorthKeeping(roundId: roundId, votingCrypto: votingCrypto) {
            LoggerProxy.error(
                "Refusing to rebuild round \(roundId): it holds a delegation commitment whose blinding factor cannot be regenerated."
            )
