...
struct MigrationCoordFlow {
    ...
    enum Action {
        ...
        /// R6) — `nil` when the run needed no split, or when the SCHEDULE store failed (the split
        /// store failing takes an earlier, separate abandon path — see the `.run` effects in
