...
enum RecoveryDeviceE2E {
    ...
    /// Always on. It used to be gated on a marker file planted beside a
    /// fixture generated by a Python script, which meant the suite skipped
    /// unless someone ran the script first, and an end-to-end test that
    /// quietly passes because nobody planted its data is worse than none.
    /// The suite now builds its own database, so there is nothing to gate on.
...
struct DelegationRecoveryDeviceE2ETests {
    ...
    ///
    /// Planting here rather than from a shell script is what leaves ONE
    /// implementation of the fixture. It also removes the whole apparatus that
    /// existed only to bridge the two: a generator script, a marker file to
    /// gate on, and a container path that had to be re-resolved because
    /// reinstalling migrates it.
    ///
