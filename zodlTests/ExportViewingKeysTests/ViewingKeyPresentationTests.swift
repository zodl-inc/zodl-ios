...
struct ViewingKeyPresentationTests {
    ...
    func viewingKeyLayouts() async throws {
        ...
        for renderState in RenderState.allCases {
            for configuration in configurations {
                ...
                let png = try await render(
                    ...
                    expectsSheet: renderState == .consent
                )
                Attachment.record(
                    [UInt8](png),
        ...
        for renderState in [RenderState.revealedQRDetail, .revealedStringDetail, .qrFailure] {
            let store = try await makeRenderStore(renderState)
            let png = try await render(
                store: store,
        ...
        if scrollToBottom {
            let scrollView = try #require(
                scrollViews(in: window)
                    .filter { $0.bounds.height >= window.bounds.height * 0.5 }
                    .filter { $0.bounds.width >= window.bounds.width * 0.95 }
                    .max { scrollableHeight($0) < scrollableHeight($1) }
            )
            let bottomOffset = scrollableHeight(scrollView)
            #expect(bottomOffset > 0)
            scrollView.setContentOffset(CGPoint(x: 0, y: bottomOffset), animated: false)
            await nextMainRunLoopTurn()
            window.layoutIfNeeded()
            #expect(scrollView.contentOffset.y >= bottomOffset - 1)
        }
