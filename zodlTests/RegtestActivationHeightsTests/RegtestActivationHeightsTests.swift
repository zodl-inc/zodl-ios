...
//  zodlTests
//
//  Covers the Ironwood regtest dev-hook wiring (MOB-1469, ported from the MOB-1455 reference
//  branch): custom NU activation heights, the fixed regtest endpoint, and regtest handling across
//  ZcashSDKEnvironment and the payment-URI parser. Read-only statics throughout -> no `.serialized`.
//
...
@Suite struct RegtestActivationHeightsTests {
    ...
    @Test func regtestNetworkCarriesCustomActivationHeights() {
        ...
        // Mainnet identity (u1… addresses, chainName "main") with custom heights — the form the
        // migration FFI and the dev server's consensus checks expect (reference commit c7f5da0f).
        #expect(network.customNetworkBase == .mainnet)
    }
