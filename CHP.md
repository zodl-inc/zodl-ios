...
| 2026-08-13 (6) | **FINDING #10 FIXED (`b4ab53a5`) — delegation retry rebuilt a write-once PCZT; the pipeline now resumes persisted setup.** Live wedge on a fresh post-reset wallet (own round `3d49bf94`, bundle=0): "refusing to overwrite pczt_sighash". Mechanism (investigator-traced, cited): the crate persists `pczt_sighash` write-once per (round, wallet, bundle) at delegation-build time (storage/queries.rs:795-800) and builds are randomized (action.rs:350/:427/:477/:525) ⇒ any second build mismatches ⇒ permanent wedge (in-session hard; restart heals only single-bundle rounds via prepareFreshRound at full re-prep cost). Two arming variants, one root: (a) background precompute builds+stores then dies silently (PIR fetch, cancel) ⇒ FIRST tap wedges — Lukas's case, his skip-sheet dwell gave it time to arm; (b) attempt-1 build, mid-prove failure, Try-again rebuilds. The gap was a PREDICATE: build-skip at coordinator :3678 tested in-memory `delegationPrecomputeStatus == .ready` while the pipeline's own persisted-setup probe (signDelegationRequest succeeds ⇔ stored sighash+alpha) was computed three lines up and discarded; `buildAndProveDelegation` (operations.rs:852-1069) resumes deterministically from stored setup — "must reproduce the values already signed instead of sampling new randomness" (:1005-1013), zero sighash writes. Fix: predicate widened to `delegationPrepared OR cachedSignature != nil` (+house comment), VotingErrorMapper fingerprint pattern ("refusing to overwrite" → existing out-of-sync retry copy, zero new strings), 3 pinning tests (resume-without-rebuild event-order pin, no-setup-still-builds, proof-complete short-circuits build AND prove). Gates: testnet build 0-err; `━ Test run with 1287 tests in 187 suites passed after 36.470 seconds with 2 known issues.` (+3 exactly, 0 ✘); lint 0 new — orchestrator's :3704 suspicion REFUTED by line-mapped diff (all 43 warnings pre-existing, uniformly shifted +10). Adjudicated NOT the cause: vote path (vote::commit already resume-safe via commit_from_recovery), cross-wallet leakage (reset wipes the exact path the flow opens; walletId primed from non-nil account at the sole mount site, SettingsCoordinator :251-258), the skipped question (bundles = delegation note-chunks, not questions; skip flow verified clean: drafts-only loop :1841, consent sheet :1399-1422, no auto-pick, synthetic abstain is explicit-choice-only). Un-wedges stuck devices in place — retry resumes stored setup; round 3d49bf94 = the natural acceptance case. RESIDUALS (wave 2, unscheduled): precompute lane still builds unconditionally (:2098-2122) — post-fix benign (surfaces only as silent precompute-failed status), probe-before-build would quiet it; Keystone rebuild-after-backout under the same guard needs adjudication — crate ships `reset_voting_session_state` → `clear_unsigned_delegation_setup_fields` (preserves signed and tx-hash bundles) designed for exactly this but unexposed via FFI (investigator: new FFI needed; executor read the same facts as no-action — resolve at wave-2 scoping). |
| 2026-08-13 (5) | **8O's ADVERSARIAL CHIP EXECUTED (Task 8P `aa8dc9e0`) — swallowed local-record failures hardened; three refutations collected en route.** The gap: both share-delegation lanes swallowed `recordShareDelegation` (local SQLite write) failures after the server already ACCEPTED the share — a local write fault could produce a completion record over locally-invisible shares (the stranded shape via storage instead of network). Investigation verdicts, all code-evidenced: **SQLITE_BUSY structurally impossible** (THREE stacked serialization layers: Swift DatabaseActor → NSLock in VotingRustBackend → Rust `Mutex<Connection>`; no busy_timeout needed); **iOS file-protection eviction theory REFUTED** (voting.sqlite3 uses default `CompleteUntilFirstUserAuthentication`, not `.complete`); **retry-duplicate theory REFUTED** — the SQL is a nullifier-guarded UPSERT (`ON CONFLICT ... DO UPDATE ... WHERE nullifier matches`, queries.rs:2695), deterministic nullifier ⇒ retries silently idempotent. Real remaining classes: disk-full/I-O (rare, genuine) + one NEW class surfaced — `databaseNotOpen` on a DB close/reopen race (low-moderate). Fix: record-ALL-then-throw at both sites (later shares' records never lost to an earlier fault; first failure thrown to the per-proposal catch → failCount → completion blocked); `markVoteSubmitted` deliberately kept unconditional (on-chain fact stays separate from bookkeeping, per 8F's own design). +2 REAL tests on the recovery lane via the `runDelegationPipeline` injectable pattern (record-past-failure-then-throw + happy-path pin). Gate: build 0-diag + `━ Test run with 1284 tests in 187 suites passed` (+2 exactly). ⚠ Debt (reported, not improvised): 8F/8K predicates are BUNDLE-granular — a partially-recorded multi-share bundle's missing share never re-records locally (severity LOW: server holds it, tally unaffected); share-granular fix = recorded-count vs `recoverableShareIndices` count, future task. |
| 2026-08-13 (4) | **FINDING #8 REMAINDER ADJUDICATED: DEV-ONLY — no routing change (Lukas's conditional ruling executed by verification, Task 8O, zero code).** Ruling: fix only if real users can reach votes-on-chain/shares-undelivered behind the read-only review routing. Verification (all file:line-evidenced): `state.voteRecords[roundId]` has **exactly ONE writer** — `reduceBatchSubmissionCompleted` (coordinator:2502) — unconditionally gated on `failCount == 0 && persistedFailureCount == 0` across the ENTIRE batch (incl. 8K's recovery-folded drafts); record written once at batch end ⇒ crash/kill windows leave nothing; no other writer exists (round-status, migration, review-store all read-only/inert). **Prime suspect REFUTED**: 8L's stop-semantics change (4xx no longer stops the batch) doesn't matter — `failCount += 1` and `.batchVoteFailed` fire BEFORE the stop check, unconditionally; pinned by pre-existing test `batchSubmissionCompletedFailsWhenVoteErrorsExist`. The completion gate itself predates CHP (`db303bc6` "Require complete durable voting submissions") — the only mechanism that EVER produced the bad state was the pre-8F skip-predicate, fixed at `797b050f`; Lukas's `3d49bf94…` = legacy dev artifact, left as-is. Adversarial bonus (out of scope, chipped for later, task_2b497e13): a SUCCESSFUL share POST's local `recordShareDelegation` write failure is swallowed (log-only) in both lanes — the same stranded shape via local-DB fault; very low probability. **Task #286 CLOSED.** |
| 2026-08-13 (3) | **VALAR CHAIN v1.3.0 RESPONSE — no breakage on rc.5; forward-compat parse shipped (Task 8N `14d4cdd7`); crate-bump question posed to Adam.** Adam's heads-up: testnet on v1.3.0-rc1 (mainnet next day), "only breaking change = new dynamic-config field for polynomial length," Vizor reference = chainapsis/vizor-wallet#506. Measured: the live stage config ALREADY serves `pir_layout.poly_len: 4096`; our Codable decode ignores unknown keys (no parse break); rc.5 has NO poly_len concept (YPIR degree implicit) BUT `supported_versions.pir` still `["v0"]` and **Vizor's own bump is a DRAFT gated on the PIR server "reporting the matching polynomial length"** — i.e. the field landed ahead of server enforcement, so rc.5 keeps working today. Key asymmetry: Vizor's "easy update" rides crate **3.0.0-rc.2** (polyLen in PirLayout + auth v2 + new delegation invariant) — for us that's the rc.6+/3.x migration, NOT this week's PRs. 8N: `polyLen: UInt32?` parsed (optional; pre-v1.3.0 configs decode nil), doc-commented DO-NOT-CONSUME on the rc.5 pin (mirror-class lesson from findings #3/#4/#7), 4 constructor sites audited untouched, fixtures assert both directions; targeted gate (build + VotingServiceConfigTests) green; pushed → PR #1988. **Question sent to Adam (Lukas): does v1.3.0 keep serving PIR-v0 clients + auth-v1 rounds through the scheduled polling session, or is the 3.0 family mandatory?** His answer decides whether the crate-bump campaign schedules pre-session. |
| 2026-08-13 (2) | **RECOVERY-POLISH PACKAGE SHIPPED — findings #8 (mechanism) + #9 CLOSED, error-masking CLOSED; one routing-policy question left open.** Four parallel delegate lanes on the reviewed branch: **8L `dcc46d39`** — the share fan-out path had NO error classifier at all (per-task catch erased errors before pruning; `shouldTryNextVoteServer` belongs to the sequential path and treats all httpErrors as retryable): new `isFatalShareRejection` = 4xx aborts the whole delegation after ONE attempt and the server's verbatim message reaches the dialog (mapper chain proof-read); 5xx + transport keep prune/failover; 4 tests pin all three behaviors. **8K `b934653d`** — finding #8's gate was TRIPLE-layered (view `.disabled(drafts.isEmpty)`, `canStartSubmission`, retry's raw `.send` bypass): all now key off `RoundSession.hasPendingSubmissionWork` = drafts ∪ undelivered-share proposals (computed at all 5 hydration sites, 8F's exact records×delegations pairing); recovery-only proposals fold into the batch loop as synthetic drafts (choice from vote records; verified `choice` is never even read in recovery); 3 tests. ⚠ **OPEN REMAINDER (task #286): rounds already holding a pre-8F false-positive persisted VoteRecord route to the read-only review screen (no CTA by design) — Lukas's `3d49bf94…` likely stuck there; fix = routing-policy ruling ("done" should require shares delivered), small diff once ruled.** **8M SDK `6f6018b2`** (full gates: cargo 74/0 · nm = exactly +1 symbol `zcashlc_voting_recoverable_share_indices` · 3-arm FFI rebuild, symbol verified per-slice · OfflineTests 873/0) + **8M app `8dfb21c7`** — finding #9 closed: recovery enumerates the crate's `recover_payloads` slicing via the new passthrough instead of guessing `singleShare ? 1 : numOptions`; guess params deleted (contained); crate tests use non-sequential indices to prove no hidden 0..<N. Also en route: 8L surfaced **zodlTests is entirely unlinted** (stale `included:` globs still say secantTests — spun off as a standalone chip, running in Lukas's separate session). **JOINT BATCH GATE over the package (+ Lukas's `a34f5453` version bump): both builds 0-diagnostic + `━ Test run with 1282 tests in 187 suites passed` (+7 = exactly the new tests; 2 pre-existing known issues).** Pushed; PR #1988 updated. SDK hand-push of `6f6018b2` → PR #1962. |
| 2026-08-13 | **RE-BASED ON MOVED MAINS + BOTH PRs OPENED — the campaign enters review.** Recon: SDK main UNMOVED since our merge (0 commits — branch PR-ready as-was); zodl main +44 (release 3.9.2 (2), MOB-1697 Orchard-spend warning, notification debug work), sole file overlap = `project.pbxproj`. zodl merge `f0028f69`: **clean auto-merge, zero conflicts**, verified semantically (all 7 `VOTING_ENABLED` entries intact incl. zodlTests Debug; version fields took main's 3.9.2 (2)). Full gate over the merged tree: both builds SUCCEEDED / 0 compiler diagnostics + `━ Test run with 1275 tests in 187 suites passed` (main's 26 tests + 2 suites absorbed; 2nd "known issue" arrived with main's own tests, zero ✘). Pushed. **PRs: SDK = [zcash/zcash-swift-wallet-sdk#1962](https://github.com/zcash/zcash-swift-wallet-sdk/pull/1962)** (tip `aec99172`, Lukas hand-pushed; body flags the main regression this re-lands) · **zodl = [zodl-inc/zodl-ios#1988](https://github.com/zodl-inc/zodl-ios/pull/1988)** (tip `f0028f69`; body = summary + repo checklist untouched, opens + rc.6-removal markers declared). Lukas's lane: Valar contact (rc.6 → drop the two injections) + Michal hygiene note (inert slipstream-internal patch — verified inert again 08-13: patch key `slipstream-core` matches no package; lock resolves `zodl-slipstream 0.1.1` from crates.io registry). |
| 2026-08-12 (9) | 🏆 **FIRST COMPLETE END-TO-END SOFTWARE VOTE IN CHP HISTORY — then wrap-up sealed.** On Harry's stage round `bac99e79…`: delegation registered + confirmed (`98B65A70…`) → proposal 1 voted + confirmed (`662A125B…`) + **all 16 shares accepted** → proposal 2 voted + confirmed (`19189A1C…`) + **all 16 shares accepted** — 32 share deliveries load-balanced across both helper servers, zero rejections, completion screen reached. Every gate-7 fix is in that log: 4C ironwood witness, 8B network threading, 8C/8D/8H/8I wire repairs, 8E slice, 8F recovery routing (live-verified en route: `submitted=[0] hasShare=[] decision=go` → recovery lane). **Two findings registered from the success, both open for next sitting:** #8 = a partially-completed round (votes on-chain, shares undelivered — Lukas's own `3d49bf94…`) leaves the submit CTA DEAD (no `submitAllDraftsTapped` entry fires; the gating doesn't recognize the state as retryable; #6's cousin); #9 = the recovery lane's 9B `numOptions` bound now EMPIRICALLY under-delivers (server accepted all 16 built shares; recovery would send 4) — fix = mirror the crate slice as 8E did, likely via an SDK count/list passthrough. Wrap-up: 8J `72c9a926` removed all 31 `__CHP` prints (scaffolds collapsed byte-identical to pre-8G, lint net −11); **BATCH GATE over the full 6-commit fast-lane stack: testnet + AppStore builds SUCCEEDED / 0 compiler diagnostics + `━ Test run with 1249 tests in 185 suites passed` (1 documented known issue)**; stack pushed. Day ledger: **9 findings, 8 fixed same-day, 2 new registered, zero orchestrator-authored lines, 3 on-chain votes + 1 complete ceremony.** |
| 2026-08-12 (8) | **GATE 7 FINDING #7 → ROOT-CAUSED VIA LUKAS-CONVENTION INSTRUMENTATION, FIXED IN TWO MEASURED STEPS (Tasks 8G/8H/8I).** Share POSTs failed on all servers; the fallback PRUNES on any error and converts exhaustion into the localized "unable to reach any vote server / check your internet" copy — **masking the real rejection** (error-masking registered for the copy/UX debt list; 5s fast-timeout and Tor-route theories were RULED OUT by the same probe: servers healthy at 0.4s, `route=direct`). Task 8G (`e6fdba96`): 30 `__CHP`-prefixed prints per Lukas's debugging convention across the share lane + retry guards. First filtered log delivered the verdict verbatim: `400 vote_round_id: expected 32 bytes, got 0` — and the crate source confirmed **rc.5's `VoteShareWire` carries NO `vote_round_id` field at all** (its siblings `DelegationSubmissionWire`/`VoteCommitmentWire` both do) while the stage `/shares` endpoint requires it → **crate↔server wire-contract gap, report to Valar**. Task 8H (`3a21277b`): inject app-side from `roundIdHex` via a STRICT hex validator (no lenient parsers — finding #3's lesson), base64 like the sibling wires. Next log: `400 encoding/hex: invalid byte 'P'` → the shares endpoint alone hex-decodes the field (delegate-vote/cast-vote take base64 — **endpoint asymmetry, measured live, second Valar item**). Task 8I (`59034ed8`): send the validated 64-char hex verbatim. Also live-proven en route: votes #2 and #3 confirmed on-chain (`2C6CC632…` on Lukas's own round), and the fresh-round experience after round rotation behaves as designed. |
| 2026-08-12 (7) | **GATE 7 FINDING #6 → FIXED (Task 8F, zodl `797b050f`, FIRST FAST-LANE FIX) — submitted vote with orphaned shares was unreachable by every retry lane, and worse: the retry showed a FALSE-POSITIVE SUCCESS.** Post-8E tap: CTA "did nothing" — actually did worse: the 8E-era failure had already run `markVoteSubmitted` (order: commit → submit → storeVoteTxHash → confirm → **mark** → shares), so the retry's submitted-bundle skip `continue`d past the only bundle AND still fell through to the `.completed` green-check "Done" receipt while zero share delegations existed; crash-recovery lane = unmarked votes only, resubmission lane = recorded delegations only ⇒ orphaned shares had NO owner. Systemic note for the register: completion bookkeeping keyed off "loop didn't throw", not "work verified done". Fix (+12/−1): skip predicate = submitted AND ≥1 recorded share delegation for the proposal; submitted-but-shareless falls through to `tryRecoverInflightVote`. Executor verification highlights: cached-hash lookup traced to crate SQL (`queries.rs:2430`, plain SELECT — no submitted filter, no adjustment needed); **idempotency proven at crate-source level** (`mark_vote_submitted` same-hash re-call = `check_text_conflict` pass, crate's own test at operations.rs:3426; `confirm_vote_submission` doc "repeated calls accepted" + `advance_van_position_in_tx` no-ops, same-position re-record accepted). Tests: none addable honestly — **the entire vote-cast/share FFI surface has zero zodlTests coverage** (the one reducer test bails at the serviceConfig guard) → debt register. **PROCESS: fast-lane protocol adopted (Lukas)** — per-fix gate = testnet build + lint only (~6 min vs ~25); AppStore build + full suite + pushes batched every ~3 fixes/session end; SDK/Rust/test-target changes keep full stack. 8F gate 1: BUILD SUCCEEDED, 0 compiler diagnostics; gates 2+3 deferred to batch. NOT pushed (batch boundary). |
| 2026-08-12 (6) | **GATE 7 FINDING #5 → FIXED SAME DAY (Task 8E, zodl `bf2051c0`) — F3's unfinished sibling; and THE VOTE IS ON CHAIN.** Post-8D tap: delegation registration ACCEPTED (**#4 live-verified**), cast-vote tx submitted + confirmed on-chain + recorded — the voting act itself is cryptographically complete; failure moved to the tally-shares handoff: `recover_wire_json failed: share_index 1 not found in vote recovery bundle`. Root cause: crate `commit` returns the FULL enc-share list but `recover_payloads` slices the persisted bundle to the FIRST share when `single_share` (`share.rs:148-160`); app `singleShare = activeSession.isLastMoment` (plausibly TRUE on the short test round); **Task 9B installed the bound in the crash-recovery lane only — the NORMAL lane kept iterating the full list** and asked for index 1. Fix: position-based prefix mirror of the crate's own slice (`singleShare ? prefix(1) : all`), +6/−1, comment citing share.rs:148-160; recovery lane untouched. **Audit: all 3 `recoverWireJson` call sites classified — normal lane FIXED, 9B lane SAFE, resubmission lane SAFE BY CONSTRUCTION** (only records indices post-success; atomic abort leaves no partial rows) — zero candidate findings. Bonus reads: `if singleShare` branch = submitAt anti-correlation stagger skipped for one share; 9B's share.rs:148-188 citation re-verified accurate. Gates: both builds SUCCEEDED (zero compiler diagnostics; executor distinguished SwiftProtobuf `*DecodingError.swift` filenames and an `'Error: %@'` xcstrings content warning from real errors) + `━ Test run with 1249 tests in 185 suites passed` (1 documented known issue), voting suites confirmed by name. Next tap enters the CRASH-RECOVERY lane by design (vote tx cached) — F3/9B's first live exercise: recover → shares → confirmation. |
| 2026-08-12 (5) | **GATE 7 FINDING #4 → FIXED SAME DAY (Task 8D, zodl `e26f16fe`) — and the mirror-staleness class is now CLOSED by measurement.** Post-8C re-vote: `vote_round_id` ACCEPTED (**#3 live-verified**); next server 400 = `tx1 effects must be 821 bytes, got 0`. Cause, proven in one read: the hand-built delegate-vote body (VotingAPIClientLiveKey `submitDelegation`) simply OMITS `tx1_effects` — the April-era body predates the Ironwood-era wire field; the crate emits it (`DelegationSubmissionWire`, base64, 821 bytes decoded), the SDK DTO carries it (`tx1Effects`, CodingKey verified), `DelegationRegistration` never threaded it. Fix: verbatim-String threading per the 8C precedent, +8 lines / 4 files (field placed in crate wire order; 821-byte test fixture matching the documented decoded size). **The class-closer: executor produced the authoritative crate-wire-vs-body key diff — ALL 9 crate keys now present, NO candidate finding #5.** Sole extra key = `sighash` (SDK's own doc: "never belonged in the submission body — the server derives the signing digest itself"; server tolerant so far) → debt register, alongside the endgame fix: POST the crate's `to_wire_json` verbatim (kills the whole mirror class, incl. the cast-vote body). Gates: testnet + AppStore SUCCEEDED (5 cosmetic Swift-build-system `DecodingError` noise lines in the testnet log, investigated: batch-compile-driver hiccup on unrelated files, zero compiler diagnostics) + `━ Test run with 1249 tests in 185 suites passed` (the 1 documented known issue). Process note: the executor treated orchestrator-relayed gate verdicts as UNTRUSTED and independently re-verified every full log before proceeding — correct behavior, worth keeping. |
| 2026-08-12 (4) | **GATE 7 FINDING #3 → FIXED SAME DAY (Task 8C, zodl `44a89b7e`) — and the crypto pipeline is now LIVE-PROVEN END-TO-END.** Post-8B tap: polls loaded, Lukas voted, and the ENTIRE heavy path ran — witness generation anchored to the Ironwood root (**finding #1 live-verified**), wallet-network check passed (**#2 verified**), PIR precompute + delegation proof + hotkey ceremony all green, voting power rendered (99,875 ZEC). Failure came at the first WIRE hop: the `.authorizing` phase = the delegation registration POSTed lazily at vote time (coordinator:1810 — so delegation had never been server-accepted; earlier "voting power" was local eligibility), rejected `400: vote_round_id must be exactly 32 bytes, got 2`. Mechanism proven in code: crate-2.0 `wire_codec.rs` emits `vote_round_id` as `b64_hex` (**base64 on the wire**, like every sibling field) and the FFI returns crate wire JSON verbatim (rule 3 ✓) — but the app parsed that string **as hex** (`Data(hexString:)`, LiveKey:431), and the parser is lenient (silently keeps accidental hex-looking pairs) → 2 garbage bytes for this round's id. Both halves inherited from April (`bd2c2aba`/`4af9b951`) and CORRECT in the crate-1.0 hex-wire era; T2's de-mirror adopted the crate's base64 wire and the assumption went silently stale — String→String, invisible to the compiler and to every mock; only a live server could catch it. Fix (delegate-authored): `DelegationRegistration.voteRoundId` `Data`→`String` **base64-verbatim per the struct's own `signedNoteNullifier` precedent** (doc comment mirrored), hex conversion deleted, wire body places it verbatim, now-dead `Data(hexString:)` extension removed, one test fixture faithfully re-encoded; producer/consumer audit: exactly one of each + one test helper, `submitVoteCommitment`'s hex path untouched-and-verified (its input genuinely is the app-side hex roundId). 4 files +7/−20. Gates: both schemes 0-error; zodlTests `━ Test run with 1249 tests in 185 suites passed` (the 1 documented known issue), both live voting suites named. ⚠ Debt register: the cast-vote body is an app-side wire mirror (crate offers `to_wire_json`) — correct today, rule-3 cleanup candidate. Re-tap pending. |
