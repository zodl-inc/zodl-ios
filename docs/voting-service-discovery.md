...
2. **Dynamic config** — fetched from the static config's `dynamic_config_urls` in order, currently the [`voting.valargroup.dev` gateway](https://voting.valargroup.dev/prod/dynamic-voting-config.json) then the GitHub-raw copy. Per the publisher's client semantics, the walk falls through to the next mirror only on a transport failure or a 5xx response — never because of content: a 4xx, or a response that fails to decode or validate, surfaces immediately. GitHub-raw branch URLs are fetched with a unique cache-busting query token because GitHub's CDN caches branch paths for ~300 s regardless of request cache headers.
...
Implementation lives in [`VotingAPIClientLiveKey.swift`](../secant/Sources/Dependencies/VotingAPIClient/VotingAPIClientLiveKey.swift) (`fetchServiceConfig`, `fetchDynamicConfigData`) and [`StaticVotingConfig.swift`](../secant/Sources/Dependencies/VotingModels/StaticVotingConfig.swift) (`loadFromNetworkWithFailover`).
